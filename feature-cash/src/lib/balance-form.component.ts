...
@Component({
  ...
  template: `
    <form
      [formGroup]="formGroup"
      (ngSubmit)="onSubmit()"
      class="w-full flex justify-between items-baseline leading-normal"
    >
      <label>Saldo</label>
      ...
      <div>
        <button
          type="submit"
          class="text-white bg-gray-700 hover:bg-gray-800 focus:ring-gray-300 focus:ring-4 focus:outline-none  font-medium rounded-lg text-sm p-2 text-center   "
        >
          <bombos-icon name="equals" />
        </button>
      </div>
    </form>
  `,
